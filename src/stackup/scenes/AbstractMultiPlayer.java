...
abstract public class AbstractMultiPlayer extends AbstractGameScene {
    ...
    private void losersAndWinners(final int looserNumber) {
        for (int i = 0; i < numPlayers; i++) {
            if (i == looserNumber) {
                ...
                loser.spawn(new Vector2f(game[i].getX() + 115, Y + BOX * AbstractScene.length + BORDER));
            } else {
                winners[i] = new SimpleEntity(WINNER, layer);
                winners[i].spawn(new Vector2f(game[i].getX() + 115, Y + BOX * AbstractScene.length + BORDER));
    ...
    protected void hideGlass() {
        for (int i = 0; i < numPlayers; i++) {
            ...
            gamePaused[i].spawn(new Vector2f(game[i].getX() + 115, Y + BOX * AbstractScene.length + BORDER));
        }
    }
