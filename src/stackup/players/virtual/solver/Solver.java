...
public class Solver {
    ...
    private final int SIZE = WIDTH - AbstractScene.length + 1;
    private final String ROTATES[];
    ...
    public Solver(final AbstractGame game, final boolean moveDown) {
        ...
        ROTATES = new String[AbstractScene.length];
        for (int i = 0; i < AbstractScene.length; i++)
            ROTATES[i] = getRepeat(i, 'C');

        final int size = WIDTH - AbstractScene.length;
    ...
    private String rotate(final IGlass glass, final int i) {
        for (int r = 0; r < i; r++)
            glass.rotate();
    ...
    private void findSolutionRecursively(final IGlass glass, final StringBuilder result) {
        ...
        for (int j = 0; j <= shift.getSpace(); j++)
            for (int i = 0; i <= avail; i++) {
                ...
                currResult.append(rotate(virtualGlass, i));
