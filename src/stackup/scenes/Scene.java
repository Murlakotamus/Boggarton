...
public enum Scene {
    ...
    public AbstractScene createScene() {
        ...
        case ONE_PLAYER_GAME:
            return new OnePlayer(WIDTH, HEIGHT, FORECAST, AbstractScene.length);
        case ONE_PLAYER_DEMO:
            return new OnePlayerDemo(WIDTH, HEIGHT, 2, AbstractScene.length);
        case PLAYER_VS_COMP:
            return new PlayerVsComp(WIDTH, HEIGHT, FORECASTS, AbstractScene.length);
        case TWO_PLAYERS_GAME:
            return new PlayerVsPlayer(WIDTH, HEIGHT, FORECASTS, AbstractScene.length);
        case TWO_PLAYERS_DEMO:
            return new TwoPlayersDemo(WIDTH, HEIGHT, new int[] { 3, 2 }, AbstractScene.length);
        case OUTRO:
            return new Outro();
        default:
            return new About();
